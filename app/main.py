...
async def get_recommendation(request: QueryRequest):
    try:
        ...
        if not distances or distances[0] > 0.45 or (distances[0] > 0.3 and not is_car_related):
            return RecommendationResponse(
                query=request.description,
                ai_answer="عذراً، أنا متخصص في تشخيص أعطال السيارات فقط. سؤالك يبدو خارج نطاق تخصصي أو غير واضح ميكانيكياً.",
                source_documents=[]
            )
