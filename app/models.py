...
class RecommendationResponse(BaseModel):
    ...
    is_advice_mode: bool = (
        False  # هل الرد مجرد نصيحة عامة؟ (True تعني إيقاف وضع الطوارئ)
    )
    ...
    recommended_mechanics: List[dict] = (
        []
    )  # قائمة الفنيين المقترحين مع بيانات التواصل والموقع
    car_id: Optional[str] = None  # معرف السيارة المستخدم في العملية لربط الطلبات
    issue_summary: Optional[str] = (
        None  # ملخص فني للمشكلة للملء التلقائي في نماذج الحجز
    )
    ...
    recommended_spare_parts: List[str] = (
        []
    )  # قائمة قطع الغيار المقترحة بناءً على المشكلة
    external_links: List[dict] = []  # روابط بحث خارجية لمزيد من المعلومات
    car_brand: Optional[str] = None  # ماركة السيارة لاستخدامها في توصيات قطع الغيار
